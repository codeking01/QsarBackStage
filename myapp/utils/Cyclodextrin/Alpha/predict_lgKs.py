...
def cal_alpha_ks(gjf):
    try:
        ...
        ii_ = np.load(base_dir+'/Cyclodextrin/Alpha/ii_left.npz')  # U L
        ii_S = ii_['ii_l_s']
        X_S = X_S[:, ii_S]
        XT_S_d = np.c_[
        ...
        re_all = np.load(base_dir+'/Cyclodextrin/Alpha/result_S.npz', allow_pickle=True)
        ...
        print("失败原因：", e)
        return "Gjf文件可能有问题，请检查一下！"


if __name__ == '__main__':
    # 用Gjf文件 得到alpha环糊精的客体结合常数
    Y_pred = cal_alpha_ks('64_17_5.gjf')
